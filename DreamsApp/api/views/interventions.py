...
class InterventionCreateView(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            serializer.save(intervention_type=intervention_type, client=client, created_by=created_by, hts_result=hts_result,
                            external_organisation=external_organisation, pregnancy_test_result=pregnancy_test_result, implementing_partner=implementing_partner)
        ...
        try:
            ...
            for intervention in interventions:
                try:
                    ...
                    return Response(status=404, data={'message': 'The supplied user {} does not exist'.format(intervention['created_by'])})

                except HTSResult.DoesNotExist:
                    return Response(status=404, data={'message': 'The supplied HTSResult {} does not exist'.format(intervention['hts_result'])})

                except ExternalOrganisation.DoesNotExist:
                    return Response(status=404, data={'message': 'The supplied ExternalOrganization {} does not exist'.format(intervention['external_organisation'])})

                except PregnancyTestResult.DoesNotExist:
                    return Response(status=404, data={'message': 'The supplied PregnancyTestResult {} does not exist'.format(intervention['pregnancy_test_result'])})

                except ImplementingPartner.DoesNotExist:
                    return Response(status=404, data={'message': 'The supplied ImplementingPartner {} does not exist'.format(intervention['implementing_partner'])})
            ...
            return Response(status=201, data={'message': 'Success! Records successfully created'})
