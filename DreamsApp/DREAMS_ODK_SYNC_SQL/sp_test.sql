...
DROP TABLE IF EXISTS dreams_test.flat_dreams_enrollment;

CREATE TABLE dreams_test.flat_dreams_enrollment (
...
CREATE PROCEDURE sp_client_disability_type(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_clientindividualandhouseholddata_disability_type (clientindividualandhouseholddata_id, disabilitytype_id)
...
CREATE PROCEDURE sp_client_rep_health_known_fp_method(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_clientreproductivehealthdata_known_fp_method (clientreproductivehealthdata_id, familyplanningmethod_id)
    SELECT recordID, c.VALUE
...
CREATE PROCEDURE sp_client_drug_used_in_last_12_months(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_clientdrugusedata_drug_used_last_12months (clientdrugusedata_id, drug_id)
    SELECT recordID, c.VALUE
...
CREATE PROCEDURE sp_client_programs_enrolled(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_clientparticipationindreams_dreams_program (clientparticipationindreams_id, dreamsprogramme_id)
    SELECT recordID, c.VALUE
...
CREATE PROCEDURE sp_client_gbv_help_provider(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_clientgenderbasedviolencedata_gbv_help_provider (clientgenderbasedviolencedata_id, gbvhelpprovider_id)
    SELECT recordID, c.VALUE
...
CREATE PROCEDURE sp_client_gbv_preferred_provider(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_clientgenderbasedviolencedata_preferred_gbv_help_p1bce (clientgenderbasedviolencedata_id, gbvhelpprovider_id)
    SELECT recordID, c.VALUE
...
CREATE PROCEDURE sp_client_current_education_supporter(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_clienteducationandemploymentdata_current_educationebf4 (clienteducationandemploymentdata_id, educationsupporter_id)
    SELECT recordID, c.VALUE
...
CREATE PROCEDURE sp_client_hiv_reason_never_tested(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_clienthivtestingdata_reason_never_tested_for_hiv (clienthivtestingdata_id, reasonnottestedforhiv_id)
    SELECT recordID, c.VALUE
...
CREATE PROCEDURE sp_ct_source_of_livelihood(IN recordID INT(11), IN parentUUID VARCHAR(100))
  BEGIN
    INSERT INTO dreams_test.DreamsApp_homevisitverification_source_of_livelihood (homevisitverification_id, sourceofincome_id)
    SELECT recordID, c.VALUE
...
CREATE PROCEDURE sp_update_demographics_location()
  BEGIN
    ...
    INNER JOIN (
      ...
      sc.id subcounty_id,
      c.id county_id
    from dreams_test.DreamsApp_ward w
    INNER JOIN dreams_test.DreamsApp_subcounty sc ON sc.id = w.sub_county_id
    INNER JOIN dreams_test.DreamsApp_county c ON c.id=sc.county_id
...
CREATE PROCEDURE sp_initial_update_enrollment_staging_table()
  BEGIN
    /*Update implementing partner*/
    UPDATE dreams_test.flat_dreams_enrollment cl
    INNER JOIN (
      select code, name from DreamsApp_implementingpartner
    ) ip ON ip.code = cl.implementing_partner_id
    SET cl.implementing_partner = ip.name
...
INSERT INTO dreams_test.DreamsApp_flatenrollmenttablelog(date_started, activity) VALUES(NOW(), 'First time population of table');
SET record_id = LAST_INSERT_ID();

INSERT INTO dreams_test.flat_dreams_enrollment(
client_id,
first_name,
middle_name,
last_name,
...
LEFT OUTER JOIN dreams_test.DreamsApp_clientsexualactivitydata s ON s.client_id = d.id
LEFT OUTER JOIN dreams_test.DreamsApp_clientreproductivehealthdata rh ON rh.client_id=d.id
LEFT OUTER JOIN dreams_test.DreamsApp_clientdrugusedata dr on dr.client_id=d.id
LEFT OUTER JOIN dreams_test.DreamsApp_clientparticipationindreams p on p.client_id=d.id
LEFT OUTER JOIN dreams_test.DreamsApp_clientgenderbasedviolencedata gbv ON gbv.client_id=d.id
LEFT OUTER JOIN dreams_test.DreamsApp_clienteducationandemploymentdata edu ON edu.client_id = d.id
LEFT OUTER JOIN dreams_test.DreamsApp_clienthivtestingdata hiv ON hiv.client_id=d.id
LEFT OUTER JOIN (
SELECT
w.id as ward_code,
w.name as ward_name,
...
UPDATE dreams_test.flat_dreams_enrollment e INNER JOIN (
    SELECT
      ...
      from dreams_test.DreamsApp_clientindividualandhouseholddata i_data
      INNER JOIN dreams_test.DreamsApp_clientindividualandhouseholddata_disability_type dt ON dt.clientindividualandhouseholddata_id = i_data.id
...
UPDATE dreams_test.flat_dreams_enrollment e INNER JOIN (
    SELECT
      ...
      FROM dreams_test.DreamsApp_clientreproductivehealthdata_known_fp_method fp
      INNER JOIN dreams_test.DreamsApp_clientreproductivehealthdata rh
...
UPDATE dreams_test.flat_dreams_enrollment e INNER JOIN (
    SELECT
      ...
      FROM dreams_test.DreamsApp_clientdrugusedata_drug_used_last_12months d
      INNER JOIN dreams_test.DreamsApp_clientdrugusedata dd ON d.clientdrugusedata_id = dd.id
...
UPDATE dreams_test.flat_dreams_enrollment e INNER JOIN (
    SELECT
      ...
      FROM dreams_test.DreamsApp_clientparticipationindreams_dreams_program dp
      INNER JOIN dreams_test.DreamsApp_clientparticipationindreams cpd
...
UPDATE dreams_test.flat_dreams_enrollment e INNER JOIN (
    SELECT
      ...
      FROM dreams_test.DreamsApp_clientgenderbasedviolencedata_gbv_help_provider provider
      INNER JOIN dreams_test.DreamsApp_clientgenderbasedviolencedata gbv on gbv.id = provider.clientgenderbasedviolencedata_id
...
UPDATE dreams_test.flat_dreams_enrollment e INNER JOIN (
    SELECT
      gbv.client_id,
      group_concat(provider.gbvhelpprovider_id) AS provider_list
      FROM dreams_test.DreamsApp_clientgenderbasedviolencedata_preferred_gbv_help_p1bce provider -- DreamsApp_clientgenderbasedviolencedata_preferred_gbv_help_provider
      INNER JOIN dreams_test.DreamsApp_clientgenderbasedviolencedata gbv on gbv.id = provider.clientgenderbasedviolencedata_id
      GROUP BY gbv.client_id
  ) gbv on gbv.client_id = e.client_id
...
UPDATE dreams_test.flat_dreams_enrollment e INNER JOIN (
    SELECT
      ...
      FROM dreams_test.DreamsApp_clienteducationandemploymentdata_current_educationebf4 s -- DreamsApp_clienteducationandemploymentdata_current_education_supporter s
      INNER JOIN dreams_test.DreamsApp_clienteducationandemploymentdata ed ON ed.id = s.clienteducationandemploymentdata_id
...
UPDATE dreams_test.flat_dreams_enrollment e INNER JOIN (
    SELECT
      ...
      FROM dreams_test.DreamsApp_clienthivtestingdata_reason_never_tested_for_hiv rn
      INNER JOIN dreams_test.DreamsApp_clienthivtestingdata hiv_d ON hiv_d.id=rn.clienthivtestingdata_id
      GROUP BY hiv_d.client_id
  ) hiv on hiv.client_id = e.client_id
SET e.reason_not_tested_for_hiv = hiv.reason_not_tested_for_hiv;

UPDATE dreams_test.DreamsApp_flatenrollmenttablelog SET date_completed = NOW() WHERE id=record_id;
...
-- update current age
UPDATE dreams_test.flat_dreams_enrollment set current_age = DATEDIFF(CURDATE(), date_of_birth) DIV 365.25 where current_age != DATEDIFF(CURDATE(), date_of_birth) DIV 365.25 ;
...
insert into stag_clean_dreams_id (client_id, dreams_id, implementing_partner_id, ward_id, err)
  ...
  FROM dreams_test.DreamsApp_client c
  left JOIN (
SELECT
  id,
  implementing_partner_id,
