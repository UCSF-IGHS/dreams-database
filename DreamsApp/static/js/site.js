$(document).ready(function () {
    ...
    $('#dreams-profile-tab-control a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                var can_change_intervention = $.parseJSON(data.can_change_intervention);
                var can_delete_intervention = $.parseJSON(data.can_delete_intervention);
                console.log("can_change_intervention: " + can_change_intervention +", can_delete_intervention: " + can_delete_intervention);
                // Clear table
                ...
                $.each(ivs, function(index, iv){
                    ...
                    insertInterventionEntryInView(table_id, iv, iv_type, intervention_category_code, hts_result, pregnancy_result, false, can_change_intervention, can_delete_intervention)
                });
    ...
    function addUserActions(can_change_intervention, can_delete_intervention) {
    ...
    function insertInterventionEntryInView(table_id, iv, iv_type, intervention_category_code, hts_result, pregnancy_result, top, can_change_intervention, can_delete_intervention) { // top is a boolean for either position to insert the record
        var tabpanel_id = '#behavioural-interventions'
        switch (intervention_category_code){
            case 1001:
                if(!top)
                    $(table_id).append("<tr id='intervention_"+ iv.pk +"'><td class='name'>" + iv_type.fields.name + " " + returnValorEmpty(iv_type.fields.is_specified, iv.fields.name_specified) + "</td><td class='intervention_date'>" + iv.fields.intervention_date +  "</td><td class='comment'> "+ iv.fields.comment + "</td>" + addUserActions(can_change_intervention, can_delete_intervention) + "</tr>")
                else
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                if(status == 'fail'){
                    ...
                    iv.fields.no_of_sessions_attended = iv.fields.no_of_sessions_attended == null ? "" : iv.fields.no_of_sessions_attended;
                    if(modalMode == "new"){
                        insertInterventionEntryInView(table_id, iv, iv_type, intervention_category_code, hts_result, pregnancy_result, true, can_change_intervention, can_delete_intervention);
