...
class InterventionAPITestCase(APITestCase):
    ...
    def test_request_with_no_authentication_returns_unauthorised(self):
        test_data = self._generate_duplicates_test_data()
        ...
        self.assertEquals(response.status_text, 'Unauthorised!',
    ...
    def test_intervention_does_not_create_duplicate_records(self):
        ...
        self.assertEquals(response.status_code, 200, 'Expected 200 response code for ignored duplicate interventions')
        self.assertEquals(response.status_text, "OK")
        self.assertEquals(response.data['message'], "Duplicate Record! Record not created since its a duplicate.")
