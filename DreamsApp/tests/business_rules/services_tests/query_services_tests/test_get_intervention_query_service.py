...
class GetInterventionQueryServiceTestCase(InterventionDelegationTestCase):
    ...
    def test_when_intervention_belongs_to_user_ip(self):
        ...
        intervention_by_ip_z_to_ip_z_client_1 = test_data['intervention_by_ip_z_to_ip_z_client_1']
        query_service = InterventionQueryService(user=ip_z_user)
        intervention = query_service.get_intervention(intervention_by_ip_z_to_ip_z_client_1.id)
        self.assertEquals(intervention_by_ip_z_to_ip_z_client_1, intervention,
        ...
        query_service = InterventionQueryService(user=ip_z_user)
        intervention = query_service.get_intervention(intervention.id)
    ...
    def test_when_user_ip_has_active_delegation_from_client_ip(self):
        ...
        intervention = query_service.get_intervention(intervention_1_by_ip_y_to_ip_y_client_1.id)
        self.assertEquals(intervention_1_by_ip_y_to_ip_y_client_1, intervention,
                          'Expected queried intervention to equal intervention')
